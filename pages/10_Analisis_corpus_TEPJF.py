...
if not summary_path.exists():
    ...
    responsive_kpi_grid(
        [
            ...
            ("Constancia mayoria", int(resumen["constancia_mayoria"].sum())),
            ("Rebase/tope", int(resumen["rebase_tope"].sum())),
            ("Personas detectadas", personas["persona_detectada"].nunique() if not personas.empty else 0),
        ]
    )
    ...
    with left:
        ...
        st.plotly_chart(px.bar(temas, x="size", y="tema_inventario", orientation="h", color_discrete_sequence=["#6B1531"]), width="stretch")
    with right:
        medios = resumen.groupby("medio", as_index=False).size().sort_values("size", ascending=True)
        st.plotly_chart(px.bar(medios, x="size", y="medio", orientation="h", color_discrete_sequence=["#1E5B4F"]), width="stretch")
    ...
    if personas.empty:
        ...
        st.dataframe(personas, width="stretch", hide_index=True)
